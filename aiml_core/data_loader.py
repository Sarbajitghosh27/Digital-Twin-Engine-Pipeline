...
# 14 literature-standard sensors for CMAPSS (no fabricated Vibration/Efficiency)
CMAPSS_SENSORS = [
    "T24", "T30", "T50", "P30", "Nf", "Nc", "Ps30",
    "phi", "NRf", "NRc", "BPR", "htBleed", "HPT_coolant", "LPT_coolant"
]
...
class DatasetManager:
    ...
    def __init__(self, data_root: str = "data"):
        ...
        self.cmapss_dir = os.path.join(data_root, "CMAPSSData") # Pointing to the real directory
    ...
    def load_real_cmapss(self, dataset_name: str) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        # Map CMAPSS 21 sensors to the 14 literature-standard channels
        sensor_map = {
            "T24": "s_2", "T30": "s_3", "T50": "s_4", "P30": "s_7", "Nf": "s_8", "Nc": "s_9",
            "Ps30": "s_11", "phi": "s_12", "NRf": "s_13", "NRc": "s_14", "BPR": "s_15",
            "htBleed": "s_17", "HPT_coolant": "s_20", "LPT_coolant": "s_21"
        }
        ...
        train_df["Setting1"] = train_df["setting1"]
        test_df["Setting1"] = test_df["setting1"]
        ...
        for is_train, records_list in [(True, train_records), (False, test_records)]:
            for engine_id in range(1, num_engines + 1):
                ...
                baselines = {
                    ...
                    "Nf": 2388.0, "Nc": 9050.0, "Ps30": 47.0, "phi": 521.0,
                    "NRf": 2388.0, "NRc": 8130.0, "BPR": 8.4, "htBleed": 392.0,
                    "HPT_coolant": 39.0, "LPT_coolant": 23.3, "Setting1": 0.0007,
                    # N-CMAPSS specific
                    "alt": 10000.0, "Mach": 0.5, "TRA": 80.0, "T2": 288.0,
                    "T48": 1200.0, "P2": 14.7, "P15": 16.0, "wf": 8.0,
                ...
                for cycle in range(1, max_cycles + 1):
                    ...
                    if is_ncmapss:
                        ...
                        row["setting3"] = 100.0
                        row["Setting1"] = row["setting1"]
                        
                    for s in sensor_list:
                        if s in ["engine_id", "cycle", "regime", "RUL_actual", "alt", "Mach", "TRA", "setting1", "setting2", "setting3", "Setting1"]:
                            continue
                        ...
                        delta = 0.0
                        if fault_mode == "HPC":
                            if s in ["T30", "T50", "htBleed", "HPT_coolant"]:
                                delta = base * 0.03 * (ratio ** alpha)
                            elif s in ["P30", "Ps30", "phi", "NRc"]:
                                delta = -base * 0.02 * (ratio ** alpha)
                        else:
                            if s in ["T24", "T50", "LPT_coolant"]:
                                delta = base * 0.04 * (ratio ** alpha)
                            elif s in ["Nf", "NRf", "BPR"]:
