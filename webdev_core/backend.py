...
from aiml_core.fault_classifier import classify_fault, FaultDiagnosis
from aiml_core.reasoning_engine import generate_reasoning
...
def get_engine_metrics_and_explanations(engine_id: int, cycle: int, explain: bool = True) -> Dict:
    ...
    with db_lock:
        ...
        if not engine.is_iot_mode and cycle in engine.cache:
            ...
            # Only return cached value if we do not need explanations, or if the cached value contains non-empty attributions
            if not explain or (cached_val.get("explainers") and len(cached_val["explainers"].get("anomaly_shap", {})) > 0):
                return cached_val
        ...
        anomaly_score = max(0.0, min(100.0, (-0.50 - raw_anomaly_score) * 1000.0))
        ...
        if explain:
            ...
            if cache_key in EXPLAINABILITY_CACHE:
                ...
                def anomaly_scorer_func(x):
                    ...
                    return np.maximum(0.0, np.minimum(100.0, (-0.50 - scores) * 1000.0))
            ...
            # Classify fault using PMA attributions
            anomaly_shap_dict = result["explainers"]["anomaly_shap"]
            fault_diag = classify_fault(anomaly_shap_dict)
            result["fault_diagnosis"] = fault_diag
            from dataclasses import asdict
            result["predictions"]["fault_diagnosis"] = asdict(fault_diag)
        else:
            result["explainers"] = {
                "anomaly_shap": {},
                "rul_shap": {},
            ...
            result["fault_diagnosis"] = None
            result["predictions"]["fault_diagnosis"] = None
...
@app.get("/api/v1/engines/{engine_id}/explain")
def get_engine_explain_v1(engine_id: int, cycle: Optional[str] = None):
    """
    Returns natural language failure reasoning/argument for the engine.
    If cycle is not provided or is 'last', uses the engine's current cycle.
    """
    with db_lock:
        if engine_id not in engines_db:
            raise HTTPException(status_code=404, detail="Engine twin not found")
        ...
        if cycle is None or cycle == "last":
            target_cycle = eng.current_cycle
        else:
            try:
                target_cycle = int(cycle)
            except ValueError:
                raise HTTPException(status_code=400, detail="Invalid cycle parameter")
        ...
        state = {
            "engine_id": engine_id,
            "cycle": target_cycle,
            "rul_p50": status["predictions"]["RUL_predicted"],
            "rul_p10": status["predictions"]["RUL_p10"],
            "rul_p90": status["predictions"]["RUL_p90"],
            "health_index": status["predictions"]["HealthIndex"],
            "fault_diagnosis": status["fault_diagnosis"],
            "pma_attributions": status["explainers"]["anomaly_shap"]
        }
        
        reasoning_text = generate_reasoning(state)
        return {"reasoning": reasoning_text, "explanation": reasoning_text}
