...
# ─── Test 7: Fault Classifier & Reasoning Engine ──────────────────────────────

def test_fault_classifier_and_reasoning_engine():
    """
    Verifies that the fault classifier correctly maps top-2 sensor attributions
    and that the reasoning engine produces a structured narrative.
    """
    ...
    # 1. Test fault classifier mapping logic
    # Mock attributions for HPC Degradation: top 2 absolute are T30, Ps30
    attributions = {
        "T30": 15.0,
        "Ps30": -12.0,
        "T50": 1.0,
    ...
    diagnosis = classify_fault(attributions)
    assert diagnosis.fault_mode == "HPC Degradation"
    assert diagnosis.primary_sensor == "T30"
    assert diagnosis.secondary_sensor == "Ps30"
    assert diagnosis.confidence > 0.8
    assert "Borescope" in diagnosis.recommended_action
    
    # Mock attributions for LPT Thermal Wear: top 2 absolute are T50, LPT_coolant
    attributions_lpt = {
        "T50": 10.0,
        "LPT_coolant": -8.0,
        "T30": 0.5
    }
    diagnosis_lpt = classify_fault(attributions_lpt)
    assert diagnosis_lpt.fault_mode == "LPT Thermal Wear"
    assert diagnosis_lpt.primary_sensor == "T50"
    
    # 2. Test reasoning engine narrative template
    mock_state = {
        "engine_id": 4,
        "cycle": 120,
        "rul_p50": 45.0,
        ...
        "pma_attributions": attributions
    }
    narrative = generate_reasoning(mock_state)
    assert "Engine #004" in narrative
    assert "Cycle 120" in narrative
    assert "RUL: 45.0 cycles" in narrative
    assert "HI: 55.0% [degraded]" in narrative
    assert "Primary driver: T30" in narrative
    assert "HPC Degradation" in narrative
    assert "Borescope" in narrative
